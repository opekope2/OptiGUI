...
fun createFurnaceFilter(resource: Resource): FilterInfo? {
    ...
    val filterBuilder = FilterBuilder(resource).addGeneralFilters<FurnaceProperties>()
    ...
    filterBuilder.replaceableTextures = textures

    return FilterInfo(
        PostProcessorFilter(filterBuilder.build(), replacement),
        textures
    )
}
...
internal fun processFurnace(furnace: BlockEntity): Any? {
    ...
    return FurnaceProperties(
        ...
        biome = lookup.lookupBiomeId(world, furnace.pos),
